...
function buildAssetMaps(assetManifest) {
  ...
  for (const asset of downloadedAssets) {
    // ✅ NEW: central manifest flag
    if (asset.skipTheme) {
      skippedAssets.push({
        fileName: asset.fileName || asset.themeAssetName || "",
        reason: "skipTheme flag"
      });
      continue;
    }

    // ✅ NEW: block large non-critical assets before they ever enter theme/assets
    const size = asset.size || 0;
    if (size > MAX_THEME_ASSET_BYTES) {
      skippedAssets.push({
        fileName: asset.fileName || asset.themeAssetName || "",
        reason: `asset > ${Math.round(MAX_THEME_ASSET_BYTES / (1024 * 1024))} MB`
      });
      continue;
    }
  ...
  $("a[href]").each((_index, node) => {
    ...
    if (/^https?:\/\//i.test(href)) {
      try {
        ...
        $node.attr("href", u.pathname + u.search + u.hash);
      } catch {
  ...
  $("form[action]").each((_index, node) => {
    ...
    if (/^https?:\/\//i.test(action)) {
      try {
        ...
        $node.attr("action", u.pathname + u.search + u.hash);
      } catch {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener("click", (e) => {
    ...
    const dialog = target.closest("dialog, [role='dialog'], .coretex-dialog");
    if (!dialog) return;

    const label = (target.textContent || "").toLowerCase();
    const shouldClose =
      label.includes("no thanks") ||
      label.includes("build muscle") ||
      label.includes("lose fat") ||
      label.includes("improve") ||
      label.includes("health");

    const isOverlay = target === dialog;

    if (shouldClose || isOverlay) {
  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        entry.target.classList.add("clone-visible");
      }
    });
  }, { threshold: 0.2 });

  document.querySelectorAll("section, .block, .card").forEach(el => observer.observe(el));

  document.querySelectorAll("[data-slider], .slider, .carousel").forEach(slider => {
    let index = 0;
...
export async function generateTheme(input = {}) {
  ...
  const { byRemoteUrl, cssAssets, jsAssets, copiedAssets, skippedAssets } = await copyThemeAssets(assetManifest);
